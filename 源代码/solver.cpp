...
extern int boolCnt, clauseCnt, sumOfAns, branchRule, * ans;
extern clause* CNF;
...
int getMaxBool_Sudoku(clause* cnf)  //对于变元数已知的数独问题 不再动态分配cnt数组的存储空间
{
    int cnt[2917] = { 0 };
    for (clause* pc = cnf; pc; pc = pc->next)
        for (node* pn = pc->head; pn; pn = pn->next)
    ...
    int maxBool, maxTimes = 0;
    for (int i = 1; i <= 1458; i++)   //先找变元的反面
        if (cnt[i + 1458] > maxTimes)
            maxTimes = cnt[i], maxBool = -i; 
    if (maxTimes == 0)     //只剩下变元的正面
    {
...
int getNextBool_4(clause* cnf)            //选取在短子句出现次数最多的变元
{
    ...
    for (clause* pc = cnf; pc; pc = pc->next)
        ...
        {
            if (pn->value > 0)
                weight[pn->value] += J(pc->num);
            else
                weight[boolCnt - pn->value] += J(pc->num);
        }
...
bool DPLL(clause*& cnf, int* v, int satORsudoku)
{
    ...
    while (flag)//能找到单子句
    {
        flag = 0;
        pc = cnf;
        while (pc && !isUnitClause(pc))  //遍历寻找单子句
            pc = pc->next;
        if (pc != NULL)//找到单子句pc
        {
    ...
    else
    {
        if (branchRule == 1)
            ...
            maxBool = getMaxBool_2(cnf);
        
        else if (branchRule == 3)
            maxBool = getNextBool_3(cnf);
        
        else if (branchRule == 4)
            maxBool = getNextBool_4(cnf);
    }
    ...
    bool ans = DPLL(cnf, v, satORsudoku); //DPLL
    destroyClause(cnf);   //删除副本
    return ans;  //返回ans
}